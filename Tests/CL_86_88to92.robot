...
Check months are scrollable left to right and vice versa
    ...
    Wait Until Element Is Visible    xpath://div[contains(@class,'Profile-module__ProviderProfile--Tabs css-0')]//p[contains(text(),'My')]
...
Check if call booked its showing animation under bookings with me
    Wait Until Element Is Visible    xpath=//span[contains(@class, 'BookingForm-module__Topic--Tag css-1fhfbim') and text()='Weight']    6s
    Click Element    xpath=//span[contains(@class, 'BookingForm-module__Topic--Tag css-1fhfbim') and text()='Weight']
    Sleep    2s
...
Checking if booking in Our bookings and delete
    Wait Until Element Is Visible    xpath=//img[@src='https://images-dev.taskhuman.com/icon/profileTabIcon/our_bookings.png' and contains(@class, 'chakra-image css-0')]    5s
