...
Check Request provider flow & when user tapped on request button it should redirect to concierge chat
    ...
    Sleep    2s
    Wait Until Element Is Visible    xpath=//div[contains(@class, 'ActionMenu-module__ActionMenu--List')]    6s
    Scroll Element Into View    xpath=(//div[contains(@class, 'ActionMenu-module__ActionMenu--List')]//img)[last()]
    Wait Until Element Is Visible    xpath=(//div[contains(@class, 'ActionMenu-module__ActionMenu--List')]//img)[last()]    6s
    Execute JavaScript    document.evaluate("(//div[contains(@class, 'ActionMenu-module__ActionMenu--List')]//img)[last()]", document, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue.click()
    Sleep    2s
...
Check chat icon if tapped taking to chat screen of provider under selected skill
    Go To    https://app-dev.taskhuman.com/profile/5466
    Sleep    2s
    Wait Until Element Is Visible    xpath://img[@src='/58fa62dcc433ef9fa89b.svg' and @class='chakra-image css-0']    10s
    Click Element    //img[@src='/58fa62dcc433ef9fa89b.svg' and @class='chakra-image css-0']
    Sleep    2s
    Wait Until Element Is Visible    xpath=//span[contains(@class, 'MessageModal-module__Prase--Tag') and contains(@class, 'css-1fhfbim') and text()='AK skill']    10s
    Click Element    xpath=//span[contains(@class, 'MessageModal-module__Prase--Tag') and contains(@class, 'css-1fhfbim') and text()='AK skill']
    Sleep    3s
    Click Element    xpath://*[text()='Next']
    Sleep    2s
    Location Should Be    https://app-dev.taskhuman.com/messages/5466/AK%20skill/65e6c60aa8ea808220cb64f5
    Sleep    2s
Check if message sent by consumer showing the time of sent
    Wait Until Element Is Visible    xpath://input[@class='chakra-input InputField-module__Message--Input css-1mh8t8j' and @maxlength='300']    10s
