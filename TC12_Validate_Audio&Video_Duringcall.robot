...
Accept Call as Provider in Incognito
    ...
    Switch Browser    INC
    Wait Until Element Is Visible    xpath://*[text()='Accept']    5s
    Click Element    xpath://*[text()='Accept']
    Sleep    5s
    Wait Until Element Is Visible    xpath://*[text()='Got It!']    5s
    Click Element    xpath://*[text()='Got It!']
Turn Camera OFF And Verify Video not visible
    Sleep    3s
    Mouse Over    (//button[@data-testid="enable_btn"])[1]
    Wait Until Element Is Visible    (//button[@data-testid="enable_btn"])[1]    5s
    Click Element    (//button[@data-testid="enable_btn"])[1]    # turn camera OFF
    Sleep    2s
    ${style}=    Get Element Attribute    id=self-view-initial    style
    Should Contain    ${style}    display: flex
Turn Camera ON And Verify Video visible
    Mouse Over    (//button[@data-testid="enable_btn"])[1]
    Wait Until Element Is Visible    (//button[@data-testid="enable_btn"])[1]    5s
    Click Element    (//button[@data-testid="enable_btn"])[1]    # turn camera ON
    Mouse Over    (//button[@data-testid="enable_btn"])[1]
    Wait Until Element Is Visible    (//button[@data-testid="enable_btn"])[2]    5s
    Click Element    (//button[@data-testid="enable_btn"])[2]    # turn mic off
    Sleep    2s
    ${style}=    Get Element Attribute    id=self-view-initial    style
    ...
    Page Should Not Contain Element    xpath=//div[contains(@class,'ProviderFeedback-module__Feedback--container')]    10s
...
Open and Position Browsers
    ...
    Open Browser    https://app-dev.taskhuman.com/login    Chrome    alias=INC    options=add_argument("--use-fake-ui-for-media-stream")    options=add_argument("--use-fake-device-for-media-stream")    options=add_experimental_option("prefs", {"profile.default_content_setting_values.media_stream_camera": 1, "profile.default_content_setting_values.media_stream_mic": 1})
    Set Window Size    650    1200
...
Validate video rendered or not    # Test case to accept the call as a provider in incognito mode.
    Turn Camera OFF And Verify Video not visible
    Turn Camera ON And Verify Video visible
